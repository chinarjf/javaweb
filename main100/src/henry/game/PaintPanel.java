 ...
 * @version 1.0
 */
abstract public class PaintPanel
    extends JPanel implements PaintListener {
  ...
  protected void paintComponent(Graphics g) {
    ...
    if (allLocationImages != null) {
      for (Iterator iter = allLocationImages.iterator(); iter.hasNext(); ) {
        ...
        g.drawImage(locationimage.getImage(), Math.round(locationimage.getXPos()),
                    Math.round(locationimage.getYPos()), this);
      }
    }
    //优先显示其他组件
